...
# 执行登录
def handleLogin(user, pwd):
    driver = webdriver.PhantomJS()
    # driver = webdriver.Chrome('/home/zhuge/bin/chromedriver')

    # 定义自己的用户名密码
    username = user  # "13071131373"
    password = pwd  # "qazwsx1234"
    # 执行操作的页面地址
    ...
    driver.set_window_size(480, 760)
    driver.get(url)
    # 目前还没学会cookie怎么用先获取到吧肯定用的login
    # 获得cookie信息
    cookie1 = driver.get_cookies()
    print 'Cur0 url...', driver.current_url
    # 将获得cookie的信息打印
    print 'cookie1...', cookie1
    # 下面就开始在打开的页面中自动执行操作了
    # 根据xpath获取登录按钮
    elem = driver.find_element_by_xpath("//a[@id='cj_yaodai_loginUrl']")
    # 发送确认按钮跳转到登录页面
    elem.send_keys(Keys.ENTER)
    print 'Cur1 url...', driver.current_url
    # 休眠两秒钟后执行填写用户名和密码操作
    sleep(2)

    # elem = driver.find_element_by_xpath("//input[@id='user_name']").clear()
    elem = driver.find_element_by_xpath("//input[@id='user_name']")
    elem.send_keys(username)
    sleep(1)
    # elem = driver.find_element_by_xpath("//input[@id='user_password']").clear()
    elem = driver.find_element_by_xpath("//input[@id='user_password']")
    elem.send_keys(password)
    sleep(1)
    elem = driver.find_element_by_xpath("//a[@id='login-user-btn']").click()
    # elem.send_keys(Keys.RETURN)
    sleep(2)
    print 'Cur2 url...', driver.current_url
    ...
    if cookie1 == cookie2:
        ...
        assert u"退出" in driver.page_source, 'False Login'
    else:
        ###################################
        cookie = [item["name"] + "=" + item["value"] for item in cookie2]
        # print cookie
        ...
        print 'Success'
        assert u"退出" in driver.page_source, 'False Login'
        key = 'shanghai-lianjia-cookie'
        qe = Queue(key)
        qe.enqueue(cookiestr)
...
if __name__ == '__main__':
    # driver = initWork()
    login_list = [['13071131373', 'qazwsx1234'],
                  ['18513324358', '123456'], ]
    key = 'shanghai-lianjia-cookie'
    qe = Queue(key)
    qe.reset()
